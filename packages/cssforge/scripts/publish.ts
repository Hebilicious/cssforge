...
const main = async (): Promise<void> => {
	...
	const alreadyPublished = await isPublished[channel]();
	if (alreadyPublished) {
		console.log(
			`${channel}: ${manifest.name}@${manifest.version} is already published, skipping`,
		);
	} else if (channel === "npm") {
		// Trusted publishing supplies the OIDC credential, and provenance is
		// generated by the registry during the same exchange.
		run("npm", ["publish"], dryRun);
	} else {
		run("npx", ["jsr", "publish"], dryRun);
	}
